...
- **Phase:** 0 — done
- **Done:** P0-01 runnable Spring Boot skeleton is complete and owner-verified: Java 21 / Spring Boot 3.x / Maven; module packages `payments`, `ledger`, `compliance`, and `common` with documented responsibilities; environment-based MySQL configuration with `.env.example` and ignored `.env`; Docker Compose app + MySQL; Actuator health returning HTTP 200 with `UP`; Flyway `V1__baseline.sql` recorded successfully with `schema_marker`; Testcontainers MySQL context-load and health endpoint tests; local `mvn -B verify` green; GitHub Actions CI run #3 green at commit `a6da471`; targeted secret-history scan clean; GitHub Secret Protection and Push Protection enabled; README quickstart and IDE development loop documented. The disposable MySQL Testcontainer alone uses `sslMode=DISABLED` to avoid host/Docker clock-skew failures with its temporary self-signed certificate; Compose and production datasource settings are unchanged. Default currency remains **MYR** (ADR-0002). Existing drafts remain unchanged: P2 design spec (`docs/design/ledger-spec.md`, DRAFT, audit at P2 entry §12), P2 task cards P2-01…P2-06, P3 pitfall register/test specs (`docs/design/p3-stripe-pitfalls-and-tests.md`, DRAFT, audit at P3 entry §6), P5–P6 compliance sketch (`docs/design/p5-p6-compliance-sketch.md`, DRAFT, audit at P5/P6 entry §7), system blueprint (`docs/blueprint.md`), and session prompt kit (`docs/ai/SESSION_PROMPT.md`).
- **Next card:** Phase 1 entry ritual, then create/select `P1-01`; no Phase 1 implementation has started.
- **Open questions:** none
