...
- **Phase:** 1 — in progress
- **Done:** Phase 0 remains owner-verified and complete. P1-01 Customer CRUD is complete. P1-02 Account CRUD is owner-verified locally: ADR-0007 defines a balance-free, MYR-only Account; Flyway `V3__create_customer_account.sql` adds the Customer foreign key, scoped name uniqueness, status/currency checks, and delete restriction; layered API/application/domain/infrastructure code provides CRUD, validation, pagination, Customer/status filtering, and consistent errors without exposing JPA entities. Customer deletion is rejected while Accounts exist. Manual CRUD/error-contract checks passed, and `mvn -B verify` is green with 16 tests against Testcontainers MySQL and an executable Spring Boot JAR. P1-03 remains defined; existing P2/P3/P5–P6 design drafts remain unchanged.
- **Next card:** Finish the P1-02 publication handback: commit and push the branch, then verify GitHub Actions. Once CI is green, select `docs/ai/tasks/P1-03-openapi-postman-and-wrap.md`.
- **Open questions:** none
